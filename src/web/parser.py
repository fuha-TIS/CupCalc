    ...
    def parse_person_tag(self, person_info, person_result):
        ...
        for person in person_result:
            if person.tag == "Name":
                ...
                person_info['person_id'] = person.text
    ...
    def parse_result_tag(self, person_info, person_result):
        ...
        for result in person_result:
            if result.tag == "CCard":
                ...
                person_info['starttime'] = result.text[11:19]
            elif result.tag == 'FinishTime':
                person_info['finishtime'] = result.text[11:19]
            elif result.tag == 'Time':
                person_info['time'] = result.text
            elif result.tag == 'Position':
                person_info['position'] = result.text
            elif result.tag == "Status":
                person_info['status'] = result.text
...
class Parser():
    ...
    def __init__(self, afile):
        self.afile = afile
        self._tree = ET.parse(afile)
        self.root = self._tree.getroot()
        self.iof_version = None
        try:
            self.iof_version = int(self.root.attrib['iofVersion'][:1])
        except:
            pass
        for child in self.root:
            if child.tag == "IOFVersion":
                self.iof_version = int(child.attrib['version'][:1])

    def select_parser(self):
        if self.iof_version == 2:
            return Parser_ver2(self.afile)
        elif self.iof_version == 3:
            return Parser_ver3(self.afile)
